...
/// @notice Phase 1.1 execution + device-management behavior.
contract KeymeshWalletTest is Test {
    ...
    function setUp() public {
        ...
        wallet = new KeymeshWallet(address(this), device);
        counter = new Counter();
    ...
    function _execute(
        ...
        wallet.execute(address(wallet), block.chainid, to, value, data, nonce, EXPIRY, _sign(key, d));
    }
    ...
    function test_CalldataExecutionReachesTarget() public {
        ...
        _execute(DEVICE_KEY, 1, address(counter), 0.5 ether, abi.encodeWithSelector(Counter.increment.selector));
    ...
    function test_MultipleSequentialTransactions() public {
        ...
        _execute(DEVICE_KEY, 2, address(counter), 0, abi.encodeWithSelector(Counter.increment.selector));
    ...
    function test_UnauthorizedDeviceSignatureRejected() public {
        vm.expectRevert(abi.encodeWithSelector(IKeymeshWallet.UnauthorizedDevice.selector, stranger));
    ...
    function test_WrongWalletParameterRejected() public {
        vm.expectRevert(abi.encodeWithSelector(IKeymeshWallet.WrongWallet.selector, address(0xBEEF)));
    ...
    function test_SignatureForDifferentWalletRejected() public {
        bytes32 foreignDigest = KeymeshTx.digest(address(0xDEAD), block.chainid, 0, device2, 0, "", EXPIRY);
    ...
    function test_SignatureForDifferentChainRejected() public {
        bytes32 crossChainDigest = KeymeshTx.digest(
            address(wallet), block.chainid + 1, 0, device2, 0, "", EXPIRY
        );
